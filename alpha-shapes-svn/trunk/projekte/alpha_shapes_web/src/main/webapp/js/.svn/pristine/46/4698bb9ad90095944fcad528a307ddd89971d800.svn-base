 ...
 * 
 * @constructor
 * @param {Vector} origin point on line
 * @param {Vector} direction of line
 */
...
Line.prototype = {
     ...
     * @returns {?Vector} projection of point on this line or null if not
     *                      existent
     */
    ...
    calculateLambda: function(point) {
        ...
        // Parallel to y-Axis
        } else if ((Misc.compareWithTolerance(this.direction.x, 0) === 0)) {
            if (Misc.compareWithTolerance(this.origin.x, point.x) !== 0) {
                return null;
            }
            var lambda = (point.y - this.origin.y) / this.direction.y;
            return lambda;
        // Parallel to x-Axis
        } else if ((Misc.compareWithTolerance(this.direction.y, 0) === 0)) {
            if (Misc.compareWithTolerance(this.origin.y, point.y) !== 0) {
                return null;
            }
        ...
        // General
            ...
            if (Misc.compareWithTolerance(lambda1, lambda2) !== 0) {
                return null;
            } else {
                return lambda1;
            }
     ...
     * 
     * @param {Vector} point to check if it is contained
     ...
     * 
     * @param {Line} line to create intersection with
     * @returns {?Vector} intersection point or null if not exitent
     */
    getIntersection: function(line) {
        ...
        // calculate lambda
        if (Misc.compareWithTolerance(line.direction.x, 0) !== 0) {
