...
(function(Vector, Circle, LineSegment, ComparatorFunctions) {
    /**
     * A triangle.
     *
     * @constructor
        ...
        calcCircumcircle: function() {
            ...
            if (ComparatorFunctions.compareWithTolerance(det, 0) !== 0) {
                ...
                ls.sort(function(ls1, ls2) {
                    return ComparatorFunctions.compare(ls1.getLength(), ls2.getLength());
                });
        ...
        getMiddleAngle: function() {
            if (this.angle === null) {
                ...
                var denom = 2 * Vector.calcDist(this.p1, this.p2) *
                    Vector.calcDist(this.p2, this.p3);
                if (ComparatorFunctions.compareWithTolerance(denom, 0) === 0) {
                    this.angle = Math.PI;
                } else {
     ...
     */
    alphashape.geom.Triangle.compareByRadiusThenAngle = function(t1, t2) {
        var compR = ComparatorFunctions.compareWithTolerance(t1.getCircumcircle().radius,
            t2.getCircumcircle().radius);
        if (compR !== 0) {
            return compR;
        } else {
            return ComparatorFunctions.compareWithTolerance(t1.getMiddleAngle(),
                t2.getMiddleAngle());
        }
    };
})(alphashape.geom.Vector, alphashape.geom.Circle, alphashape.geom.LineSegment,
